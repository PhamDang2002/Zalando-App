import { ratingStar } from "@redux/slices/authSlice";
...
export default function RatingStars({ rating, setRating }) {
  const handleFilterStar = (star) => {
    if (rating === star) {
      setRating(undefined); // Nếu đã chọn rồi thì bỏ chọn
    } else {
      setRating(star); // Nếu chưa chọn thì chọn
    }
  };

  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(ratingStar(rating));
  }, [rating, dispatch]);
